...
A extração/associação Rust compartilhada por V21 e V22 admite lote cego estreito: fonte Rust para IR
...
e precedência de citação. O gate não pode usar os classificadores V21 ou V22 como oráculo,
deve incluir regressão explícita de ambos os consumidores e deve
...
### SPEC-GAP de associação compartilhada V21/V22

O recorte acima não pode incluir associação de citações. V21 aceita somente
`ref:/spec:/rationale:` e fixa janela/precedência; o histórico V22 reconhece também
`file:line`/formas sem prefixo, enquanto a IR compartilhada expõe apenas
`citation: Option<Citation>`. Não está decidido como representar evidência válida apenas
para V22 sem silenciar V21, nem há contrato perene completo da agregação V22.

Permanece delimitável apenas fonte Rust→IR estrutural: literal, span/localização, origem
em teste/tabela, scaling, `context_var` e `geometric_sink`. Associação/janela de citação e
semântica agregada V22 exigem saneamento L0 e ficam fora de recomendação funcional direta.
