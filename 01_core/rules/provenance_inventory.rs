...
fn extract_module_name(path: &Path) -> String {
    ...
    if comps[0].starts_with("01_") || comps[0].starts_with("02_") || comps[0].starts_with("03_") || comps[0].starts_with("04_") {
        if comps.len() >= 3 && (comps[1] == "entities" || comps[1] == "contracts" || comps[1] == "rules") {
...
mod tests {
    ...
    fn provenance_inventory_aggregates_multiple_files_per_module() {
        ...
        assert_eq!(viols.len(), 1, "Agrega os 2 arquivos em 1 entrada para o módulo 01_core/rules");
