...
impl<'a> HasSemanticObservations<'a> for PublicFile {
    fn semantic_observations(&self) -> &[SemanticObservation] { &self.observations }
    fn path(&self) -> &'a Path { self.path }
    fn language(&self) -> &Language { &self.language }
}

fn obs(kind: SemanticObservationKind, contract: &str, detail: &str, line: usize, column: usize) -> SemanticObservation {
    SemanticObservation { contract_id: contract.into(), kind, detail: detail.into(), line, column }
}

fn file(language: Language, observations: Vec<SemanticObservation>) -> PublicFile {
    PublicFile { path: Path::new("unicode/λ-gate.rs"), language, observations }
}
...
fn evidence(v: &Violation<'_>) -> (String, ViolationLevel, String, String, usize, usize) {
    (v.rule_id.clone(), v.level.clone(), v.message.clone(), v.location.path.display().to_string(), v.location.line, v.location.column)
}
...
fn complete_seven_by_three_matrix_and_v25_modalities() {
    ...
    assert_eq!(v23.len(), 2); assert_eq!(v24.len(), 1); assert_eq!(v25.len(), 4);
    assert_eq!(v23.iter().map(|v| v.rule_id.as_str()).collect::<Vec<_>>(), ["V23", "V23"]);
    assert_eq!(v24[0].rule_id, "V24"); assert!(v25.iter().all(|v| v.rule_id == "V25"));
    for (v, contract, detail, mode) in [
        ...
        assert!(v.message.contains(contract)); assert!(v.message.contains(detail)); assert!(v.message.contains(mode));
    }
    ...
    assert_eq!(v25.iter().map(|v| (v.location.line, v.location.column)).collect::<Vec<_>>(), [(7,8),(9,10),(11,12),(13,14)]);
}
...
fn every_public_level_is_preserved_exactly() {
    for level in [ViolationLevel::Info, ViolationLevel::Warning, ViolationLevel::Error, ViolationLevel::Fatal] {
        let f = file(Language::Rust, vec![
            obs(SemanticObservationKind::ContextNeutralArgument, "a", "d", 1, 1),
            obs(SemanticObservationKind::NeutralProjectionDestination, "b", "d", 2, 2),
            obs(SemanticObservationKind::DuplicateDecisionOwner, "c", "d", 3, 3),
        ]);
        assert_eq!(context_erasure::check(&f, level.clone())[0].level, level);
        assert_eq!(semantic_field_loss::check(&f, level.clone())[0].level, level);
...
fn evidence_order_multiplicity_permutation_unicode_empty_and_extremes() {
    ...
    assert_eq!(x.len(), 3); assert_eq!(y.len(), 2);
    ...
    assert!(x[0].message.contains("contrato λ")); assert!(x[0].message.contains("detalhe 🙂"));
    assert_eq!(x[0].location.path, Path::new("unicode/λ-gate.rs"));
}

#[test]
fn language_and_ignored_fields_are_integrally_inert() {
    let eligible = obs(SemanticObservationKind::NeutralProjectionDestination, "id", "detail", 4, 5);
    let rust = file(Language::Rust, vec![
        obs(SemanticObservationKind::ContextNeutralArgument, "ignored-a", "x", 1, 2), eligible.clone(),
    ]);
    let python = file(Language::Python, vec![
        obs(SemanticObservationKind::DirectDecisionReimplementation, "ignored-b", "🙂", usize::MAX, 0), eligible,
    ]);
    let a = semantic_field_loss::check(&rust, ViolationLevel::Info);
    let b = semantic_field_loss::check(&python, ViolationLevel::Info);
    assert_eq!(a.len(), 1); assert_eq!(b.len(), 1); assert_eq!(evidence(&a[0]), evidence(&b[0]));
}
...
fn no_cross_activation_and_empty_is_total_for_all_rules() {
    ...
    for (kind, expected) in [
        ...
        (SemanticObservationKind::NeutralProjectionDestination, (0,1,0)),
        ...
        (SemanticObservationKind::DirectDecisionReimplementation, (0,0,1)),
    ] {
        let f = file(Language::Unknown, vec![obs(kind, "", "", 0, usize::MAX)]);
        assert_eq!((context_erasure::check(&f, ViolationLevel::Info).len(), semantic_field_loss::check(&f, ViolationLevel::Info).len(), decision_ownership::check(&f, ViolationLevel::Info).len()), expected);
    }
}
